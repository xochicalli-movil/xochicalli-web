...
const UserProfile = () => {
    ...
    const isCompletedInformation = () => {
        if (userInformation.name.length <= 1 ||
    ...
    // }
    return (_jsxs(VStack, { minH: 'calc(100vh - 64px)', bgColor: 'gray.100', p: 4, children: [_jsx(Helmet, { children: _jsx("title", { children: "Perfil de usuario" }) }), !user?.emailVerified
                ? _jsx(Heading, { children: "Revisa tu correo electr\u00F3nico para activar tu cuenta" })
                ...
                //     }
                // </Box>
                : _jsxs(_Fragment, { children: [_jsx(Heading, { mt: 4, mb: 6, children: "Tu perfil" }), _jsxs(VStack, { gap: 6, children: [_jsx(UserHeaderCard, { createdAt: userInformation.createdAt, imageURL: userInformation.profilePicture, name: `${userInformation?.name} ${userInformation?.fatherSurname} ${userInformation?.motherSurname}` }), _jsx(UserInformationCards, { uid: user?.uid, hasCompletedInformation: isCompletedInformation })] })] })] }));
};
